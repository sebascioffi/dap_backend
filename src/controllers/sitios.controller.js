...
export const getSitioPorId = async (req, res) => {
    try {
        const { idSitio } = req.params;
        const [rows] = await pool.query("SELECT * FROM SITIOS WHERE idSitio = ?", [
            idSitio,
        ]);

        if (rows.length <= 0) {
            return res.status(404).json({ message: "Sitio Inexistente" });
        }
